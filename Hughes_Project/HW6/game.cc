...
namespace main_savitch_14
{
	...
	//just so I tell the users who go first
	cout << "\nRules for Keisuke:\n"
	<< "Jump over the pieces and when all the pieces are gone someone wins";
	cout << "\nBlack team goes first\n";
	cout << "O are normal checkers, X are kings\n";
	...
	display_message("Your move, (ex: F1E2): ");
	getline(cin, answer);
	return answer;
    }
	...
	// value of the best of these moves from the opponent's perspective.
    	compute_moves(moves); 
	// LEAVE COMMENTED OUT assert(!moves.empty( ));
    	best_value = INT_MIN;
    	while (!moves.empty( ))
    	{
	    future = clone( );
	...
	compute_moves(moves);
	// LEAVE UNCOMMENTEDassert(!moves.empty( ));
